...
export default function AllRoomsPage() {
  ...
  const [roomToEdit, setRoomToEdit] = useState<RentalRoom | null>(null);
  ...
  return (
    <PageContainer width="wide" className="animate-in fade-in duration-500">
      ...
      {!roomsQuery.isLoading && view === "map" && (
        <RoomMap rooms={visibleRooms} blocks={facilityBlocks} houses={houses} facilityIdFilter={facilityIdFilter} getFacilityName={getFacilityName} getFacilityId={getFacilityId} onForfeit={setReservationToForfeit} onEdit={setRoomToEdit} />
      )}
      {!roomsQuery.isLoading && view === "list" && (
        <RoomList rooms={visibleRooms} facilityIdFilter={facilityIdFilter} getFacilityId={getFacilityId} getFacilityName={getFacilityName} onForfeit={setReservationToForfeit} onDelete={handleDelete} onEdit={setRoomToEdit} />
      )}
      ...
      {roomToEdit && (
        <RoomEditModal
          room={roomToEdit}
          facilityId={getFacilityId(roomToEdit)}
...
function RoomMap({ rooms, blocks, houses, facilityIdFilter, getFacilityName, getFacilityId, onForfeit, onEdit }: { rooms: RentalRoom[]; blocks: any[]; houses: any[]; facilityIdFilter: string; getFacilityName: (room: RentalRoom) => string; getFacilityId: (room: RentalRoom) => string; onForfeit: (room: RentalRoom) => void; onEdit: (room: RentalRoom) => void }) {
  ...
  if (!facilityIdFilter) {
    ...
    return (
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {rooms.map((room) => (
          <RoomMiniCard key={room.id} room={room} facilityId={getFacilityId(room)} onForfeit={onForfeit} onEdit={onEdit} />
        ))}
      </div>
    );
  }
...
function RoomMiniCard({ room, facilityId, onForfeit, onEdit }: { room: RentalRoom; facilityId: string; onForfeit: (room: RentalRoom) => void; onEdit: (room: RentalRoom) => void }) {
  ...
  return <div className="rounded-lg border border-slate-200 bg-white p-3 transition-colors hover:border-blue-300"><div className="flex items-start justify-between gap-2"><button type="button" onClick={() => onEdit(room)} className="min-w-0 text-left"><div className="font-bold text-slate-900">{room.name}</div><div className="mt-1 flex items-center gap-1.5 text-xs font-medium text-slate-600"><span className={`h-2 w-2 rounded-full ${dot}`} aria-hidden="true" />{label}</div></button><RoomActionMenu room={room} facilityId={facilityId} onForfeit={onForfeit} onEdit={onEdit} /></div><div className="mt-3 text-sm font-semibold text-slate-800">{formatMoney(room.price)}<span className="font-normal text-slate-500">/tháng</span></div><p className="mt-1 truncate text-xs text-slate-500">{room.tenant_name || "Chưa có khách thuê"}</p><div className="mt-3 flex gap-2">{overdue ? <><Link className="flex-1" href={`/invoices?room_id=${room.id}`}><Button size="sm" variant="primary" className="w-full" icon={<Banknote size={14} />}>Thu tiền</Button></Link><Link className="flex-1" href={`/invoices?room_id=${room.id}&action=remind`}><Button size="sm" variant="outline" className="w-full" icon={<Bell size={14} />} aria-label="Nhắc nợ">Nhắc nợ</Button></Link></> : status === "maintenance" ? <Button size="sm" variant="outline" className="w-full" icon={<Wrench size={14} />} onClick={() => onEdit(room)}>Chi tiết</Button> : status === "occupied" && expiring ? <><Link className="flex-1" href={`/contracts/${room.contract_id}`}><Button size="sm" variant="primary" className="w-full">Gia hạn</Button></Link><Link className="flex-1" href={`/contracts/${room.contract_id}?action=terminate`}><Button size="sm" variant="outline" className="w-full">Trả phòng</Button></Link></> : status === "occupied" ? <><Link className="flex-1" href={`/invoices?room_id=${room.id}`}><Button size="sm" variant="primary" className="w-full" icon={<Banknote size={14} />}>Thu tiền</Button></Link><Link className="flex-1" href={`/contracts/${room.contract_id}?action=terminate`}><Button size="sm" variant="outline" className="w-full">Trả phòng</Button></Link></> : status === "reserved" ? <><Link className="flex-1" href={`/contracts/new?room_id=${room.id}&facility_id=${facilityId}`}><Button size="sm" variant="primary" className="w-full" icon={<FileText size={14} />}>Tạo HĐ</Button></Link>{room.reservation_deposit_id ? <Button size="sm" variant="outline" className="flex-1" onClick={() => onForfeit(room)}>Bỏ cọc</Button> : null}</> : <><Link className="flex-1" href={`/contracts/new?room_id=${room.id}&facility_id=${facilityId}`}><Button size="sm" variant="primary" className="w-full">Tạo HĐ</Button></Link><Link className="flex-1" href={`/deposits?room_id=${room.id}`}><Button size="sm" variant="outline" className="w-full">Đặt cọc</Button></Link></>}</div></div>;
}

function RoomList({ rooms, facilityIdFilter, getFacilityId, getFacilityName, onForfeit, onDelete, onEdit }: { rooms: RentalRoom[]; facilityIdFilter: string; getFacilityId: (room: RentalRoom) => string; getFacilityName: (room: RentalRoom) => string; onForfeit: (room: RentalRoom) => void; onDelete: (room: RentalRoom) => void; onEdit: (room: RentalRoom) => void }) {
  if (!rooms.length) return <Card className="p-8 text-center text-sm text-slate-500">Không có phòng phù hợp bộ lọc.</Card>;
  return <div className="overflow-hidden rounded-xl border border-slate-200 bg-white"><div className="hidden grid-cols-[minmax(100px,1fr)_minmax(150px,1.4fr)_minmax(130px,1fr)_minmax(135px,1fr)_42px] gap-4 border-b border-slate-200 bg-slate-50 px-5 py-3 text-xs font-bold uppercase tracking-wide text-slate-500 md:grid"><span>Phòng</span><span>{facilityIdFilter ? "Khách thuê" : "Cơ sở / khách thuê"}</span><span>Trạng thái</span><span className="text-right">Giá thuê</span><span /></div>{rooms.map((room) => { const meta = roomStatusMeta(room.status, isContractSoonEnding(room), (room as any).is_expired); const facilityId = getFacilityId(room); const facilityName = getFacilityName(room); return <div key={room.id} className="grid gap-2 border-b border-slate-100 px-4 py-4 last:border-b-0 md:grid-cols-[minmax(100px,1fr)_minmax(150px,1.4fr)_minmax(130px,1fr)_minmax(135px,1fr)_42px] md:items-center md:gap-4 md:px-5"><div><button type="button" onClick={() => onEdit(room)} className="font-bold text-slate-900 hover:text-blue-600">{room.name}</button><p className="mt-0.5 text-xs text-slate-500">{getFloorFromRoomName(room.name)} · {getRoomArea(room)} m²</p></div><div className="min-w-0"><p className="truncate text-sm font-medium text-slate-800">{room.tenant_name || "Chưa có khách thuê"}</p>{!facilityIdFilter && <p className="mt-0.5 truncate text-xs text-blue-600">{facilityName || "Chưa chọn cơ sở"}</p>}</div><div><Badge variant={meta.className.includes("green") ? "success" : meta.className.includes("blue") ? "primary" : meta.className.includes("orange") || meta.className.includes("amber") ? "warning" : meta.className.includes("red") ? "danger" : "neutral"}>{meta.label}</Badge></div><p className="text-sm font-bold text-slate-900 md:text-right">{formatMoney(room.price)}<span className="font-normal text-slate-500">/tháng</span></p><div className="justify-self-end"><RoomActionMenu room={room} facilityId={facilityId} onForfeit={onForfeit} onDelete={onDelete} onEdit={onEdit} /></div></div>; })}</div>;
}

function RoomActionMenu({ room, facilityId, onForfeit, onDelete, onEdit }: { room: RentalRoom; facilityId: string; onForfeit: (room: RentalRoom) => void; onDelete?: (room: RentalRoom) => void; onEdit: (room: RentalRoom) => void }) {
  const status = String(room.status || "vacant").toLowerCase();
  const close = (event: React.MouseEvent<HTMLElement>) => event.currentTarget.closest("details")?.removeAttribute("open");
  return <details className="relative"><summary aria-label={`Thao tác với phòng ${room.name}`} className="list-none cursor-pointer rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 hover:text-blue-600 [&::-webkit-details-marker]:hidden"><MoreHorizontal size={17} /></summary><div className="absolute right-0 z-20 mt-1 w-44 overflow-hidden rounded-lg border border-slate-200 bg-white py-1 shadow-lg"><button role="menuitem" onClick={(event) => { close(event); onEdit(room); }} className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm font-medium text-slate-700 hover:bg-slate-50"><Edit3 size={14} />Chỉnh sửa phòng</button>{status === "reserved" && room.reservation_deposit_id && <button role="menuitem" onClick={(event) => { close(event); onForfeit(room); }} className="flex w-full items-center gap-2 px-3 py-2 text-left text-sm font-medium text-amber-700 hover:bg-amber-50"><X size={14} />Bỏ cọc giữ chỗ</button>}{status === "occupied" && room.contract_id && <Link onClick={close} href={`/contracts/${room.contract_id}`} className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50"><FileText size={14} />Xem hợp đồng</Link>}{status === "occupied" && room.contract_id && <Link onClick={close} href={`/contracts/${room.contract_id}?action=terminate`} className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50"><X size={14} />Trả phòng</Link>}{onDelete && <button role="menuitem" onClick={(event) => { close(event); onDelete(room); }} className="flex w-full items-center gap-2 border-t border-slate-100 px-3 py-2 text-left text-sm font-medium text-red-600 hover:bg-red-50"><Trash2 size={14} />Xóa phòng</button>}</div></details>;
}
