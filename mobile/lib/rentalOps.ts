/**
 * TrọCare Mobile — Rental & Operations API Helpers
 * Ported from web-admin/src/lib/rentalOps.ts
 */
...
export type RoomStatus = 'vacant' | 'occupied' | 'maintenance' | 'reserved' | 'expiring_soon' | 'expired' | 'disabled';
...
export const normalizeRoomStatusValue = (status?: string | null): RoomStatus => {
  const stat = String(status || '').toLowerCase();
  if (stat === 'available' || stat === 'vacant') return 'vacant';
  ...
  if (stat === 'disabled') return 'disabled';
...
export const normalizeRoomStatus = (room: RentalRoom): RoomStatus => {
  ...
  return normalizeRoomStatusValue(room.status);
};
...
export const toContractView = (room: RentalRoom): ContractView | null => {
  ...
  return {
    ...
    applied_services_snapshot: normalizeAppliedServicesSnapshot((room as any).applied_services_snapshot),
    note: room.note || '',
...
export async function createInvoiceForContract(contract: ContractView, input: {
  ...
  return createInvoice({
    ...
    previousDebt: Number(input.previousDebt || 0),
...
export async function recordPayment(invoice: Invoice, input: { amount: number; walletId: string; date: string; method: string; collector: string; note?: string }) {
  ...
  const description = [
    `Thu tiền phòng ${invoice.room_name || invoice.room_id} T${invoice.month}/${invoice.year}`,
    input.method,
    input.collector ? `Người thu: ${input.collector}` : '',
    input.note || '',
  ].filter(Boolean).join(' · ');

  const txRes = await apiPost<any>('/transactions', {
    type: 'income',
    amount,
    description,
  ...
  let res;
  try {
    res = await apiPost<any>(`/invoices/${invoice.id}/mark-paid`, {
      paidAmount: amount,
    ...
    if (transaction?.id) {
      try {
        ...
        throw new Error('Đã tạo giao dịch nhưng cập nhật hóa đơn thất bại. Vui lòng kiểm tra sổ thu chi và hóa đơn trước khi thu lại.');
      }
    }
    throw error;
  }
...
export async function loadTradingItems(walletId: string) {
  const res = await apiGet<any>(`/trading/items?walletId=${walletId}`);
  return (res?.data ?? []) as TradingItem[];
}

export async function loadTradingStats(walletId: string) {
  const res = await apiGet<any>(`/trading/stats?walletId=${walletId}`);
  return (res?.data ?? { unsoldCapital: 0, unsoldCount: 0, realizedProfit: 0, soldCount: 0 }) as TradingStats;
}

export async function createTradingItem(input: {
  walletId: string;
  name: string;
  category?: string;
  importPrice: number;
  ...
  const res = await apiPost<any>('/trading/items', input);
  return (res?.data ?? []) as TradingItem[];
}

export async function sellTradingItem(id: string, input: { sellPrice: number; sellDate: string }) {
  const res = await apiPatch<any>(`/trading/items/${id}`, {
...
export async function deleteTradingItem(id: string) {
  const res = await apiDelete<any>(`/trading/items/${id}`);
  return res;
}
...
export async function loadAuditLogs() {
  const res = await apiGet<any>('/owner/audit-logs');
...
export async function updateService(id: string, input: Partial<{
  name: string;
  type: string;
  unitPrice: number;
  unitPriceAc: number;
