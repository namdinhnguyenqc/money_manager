-- Migration 015: Align legacy rental foreign keys with UUID parent IDs.
--
-- This fixes the partial-upgrade state where public.rooms.id has already been
-- converted to uuid but dependent columns such as contracts.room_id or
-- invoices.room_id are still bigint.
--
-- Important: existing non-null child values must already be UUID strings, or
-- the child table must be empty. Numeric legacy references cannot be mapped to
-- new UUID room IDs unless a separate legacy-id mapping was preserved.
