...
export default function BankConfigScreen({ navigation }) {
  ...
  const loadConfig = async () => {
    try {
      ...
      console.error(e);
      Alert.alert('Loi', 'Khong the tai cau hinh ngan hang');
    } finally {
      setLoading(false);
    }
  };
  ...
  const pickImage = async () => {
    ...
    if (status !== 'granted') {
      Alert.alert('Quyen truy cap', 'Can cap quyen thu vien anh de tai QR');
      return;
    }
  ...
  const handleSave = async () => {
    if (!bankId || !accountNo || !accountName) {
      Alert.alert('Thieu thong tin', 'Vui long nhap day du thong tin ngan hang');
      return;
    }
    ...
    try {
      ...
      Alert.alert('Thanh cong', 'Da luu cau hinh tai khoan nhan tien', [
        { text: 'OK', onPress: () => navigation.goBack() },
      ]);
    } catch (e) {
      console.error(e);
      Alert.alert('Loi', 'Khong the luu cau hinh');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <KeyboardAvoidingView style={styles.root} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
      {!isDesktopWeb ? <TopAppBar title="Tai khoan nhan tien" subtitle="Cai dat" onBack={() => navigation.goBack()} /> : null}
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={[styles.content, isDesktopWeb && styles.contentWeb, { maxWidth: contentMaxWidth }]} showsVerticalScrollIndicator={false}>
        {isDesktopWeb ? (
          <View style={styles.webActionRow}>
            ...
            <TouchableOpacity style={[styles.webPrimaryBtn, saving && styles.saveDisabled]} onPress={handleSave} disabled={saving}>
              {saving ? <ActivityIndicator color="#fff" /> : <Text style={styles.webPrimaryBtnText}>Luu cau hinh</Text>}
            </TouchableOpacity>
          </View>
        ) : null}
        <SurfaceCard>
          ...
          <View style={styles.inputGroup}>
            <Text style={styles.label}>Ten ngan hang (VD: ACB, VCB)</Text>
          ...
          <View style={styles.inputGroup}>
            <Text style={styles.label}>Ten chu tai khoan</Text>
            <TextInput
              style={styles.input}
              value={accountName}
        ...
        <SurfaceCard>
          <Text style={styles.sectionTitle}>Ma QR thanh toan</Text>
          <Text style={styles.note}>Nen tai len anh QR tinh de khach quet thanh toan nhanh.</Text>
            ...
            {qrUri ? (
              <View style={styles.qrContent}>
                ...
                <TouchableOpacity style={styles.changeBtn} onPress={pickImage}>
                  ...
                  <Text style={styles.changeText}>Doi anh khac</Text>
                </TouchableOpacity>
                <TouchableOpacity onPress={() => setQrUri(null)} style={styles.removeWrap}>
                  <Text style={styles.removeText}>Go bo anh (dung VietQR tu dong)</Text>
                </TouchableOpacity>
              </View>
            ) : (
              <TouchableOpacity style={styles.upload} onPress={pickImage}>
                <Ionicons name="image-outline" size={42} color={COLORS.outline} />
                <Text style={styles.uploadTitle}>Nhan de tai anh QR</Text>
                <Text style={styles.uploadSub}>Dinh dang .jpg, .png</Text>
              </TouchableOpacity>
            )}
          </View>
        </SurfaceCard>
        ...
        <TouchableOpacity style={[styles.saveBtn, saving && styles.saveDisabled]} onPress={handleSave} disabled={saving}>
          {saving ? <ActivityIndicator color="#fff" /> : <Text style={styles.saveText}>Luu cau hinh</Text>}
        </TouchableOpacity>
      </ScrollView>
