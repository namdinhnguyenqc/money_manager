  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (!statsRes.ok) {
        ...
        throw new Error(data?.message || data?.error || 'Không thể tải thống kê.')
      }
      if (!usersRes.ok) {
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-4">
        <div className="rounded border border-slate-200 bg-white p-4">
          <div className="mb-4 flex items-center justify-between gap-3">
            <div>
              <h1 className="text-xl font-semibold text-slate-950">Báo cáo Super Admin</h1>
          ...
          {stats && (
            <div className="grid grid-cols-2 gap-3 lg:grid-cols-5">
              ...
              <div className="rounded border border-slate-200 bg-slate-50 p-4">
                <div className="text-sm text-slate-500">Đăng nhập tháng này</div>
        ...
        <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
          <div className="rounded border border-slate-200 bg-white p-4">
            <div className="text-sm text-slate-500">Tài khoản chủ trọ</div>
            <div className="mt-2 text-3xl font-bold text-slate-950">{owners}</div>
          </div>
          <div className="rounded border border-slate-200 bg-white p-4">
            <div className="text-sm text-slate-500">Tài khoản Admin</div>
            <div className="mt-2 text-3xl font-bold text-slate-950">{admins}</div>
          </div>
          <div className="rounded border border-slate-200 bg-white p-4">
            <div className="text-sm text-slate-500">Tài khoản đã xóa</div>
